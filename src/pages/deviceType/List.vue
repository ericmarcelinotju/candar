...
import { get as getDeviceTypes, del as deleteDeviceType } from '@/api/deviceType'
import { useNotify } from '@/composables/use-notify'
import { DeviceType } from '@/typings/models/deviceType.type'
import { deviceTypeCreate, deviceTypeEdit } from '@/router/routes/deviceType'
...
const items: Ref<DeviceType[]> = ref([])
...
const handleSearch = (params) => {
  ...
  getDeviceTypes(params)
    .then(res => {
      items.value = res.data.device_types
...
const handleCreate = () => {
  router.push(deviceTypeCreate)
}

const handleEdit = ({ id }) => {
  router.push({ ...deviceTypeEdit, params: { id } })
}

// Delete deviceType
const loadingDelete = ref(false)
const visibleDeleteConfirmationModal = ref(false)
const deleteItem: Ref<DeviceType> = ref()
...
const copyColumns = [...store.getters['columns/deviceType']]
...
const onConfirmFieldConfig = () => {
  ...
  store.commit('columns/setDeviceType', columns)
}
const setDefaultFields = () => {
  store.commit('columns/setDefaultDeviceType')
  Object.assign(columns, [...store.getters['columns/deviceType']])
}
